...
function NavBar({isLoggedIn}) {
  ...
  useEffect(() => {
    ...
    if(userInfo) {
      ...
      const image = userInfo[0].user_image || "https://cdn.thedreamhome.click/default-avt.jpg";
      setUserImage(image);
    }
  ...
  className="w-10 h-10 rounded-full cursor-pointer ml-5" src={`${userImage}`} alt="Avatar"/>
  <div className={`${isMenuOpen ? '' : 'hidden'} absolute z-10 left-0 top-full w-[240px]   bg-white rounded shadow-lg flex flex-col`}>
    <div className='flex flex-col justify-center py-[15px] px-5 gap-y-[5px] flex-wrap'>
    <span className='text-[#1c1d21] text-sm font-medium '>{userName}</span>
    <span className='text-[#575757] text-[13px] font-normal'>{userEmail}</span>
    </div>
    ...
    <div className='flex flex-col justify-center py-[15px] px-5 gap-y-[10px]'>
      <div className='flex flex-row gap-x-[15px] flex-wrap p-2 hover:bg-[#E4E6E8]  rounded-md'>
        ...
        <Link href='/account' className='text-[#1c1d21] text-sm font-normal '>Xem trang cá nhân</Link>
      </div>
      {/* <div className='flex flex-row gap-x-[15px] flex-wrap p-2 hover:bg-[#E4E6E8]  rounded-md'>
      ...
      <div className='flex flex-row gap-x-[15px] flex-wrap p-2 hover:bg-[#E4E6E8]  rounded-md'>
        ...
        <span onClick={handleLogout} className='text-[#1c1d21] text-sm font-normal cursor-pointer '>Đăng xuất</span>
      </div>
